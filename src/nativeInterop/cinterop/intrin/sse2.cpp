...
namespace bslli_si128 {
	const unordered_map<char, function<__m128i(__m128i)>> map{
...
__m128i sse2_bslli_si128(__m128i a, char imm8) {
	if (imm8 > 15) imm8 = 16;
	return bslli_si128::map.at(imm8)(a);
}
...
__m128i sse2_bsrli_si128(__m128i a, char imm8) {
	if (imm8 > 15) imm8 = 16;
	return bsrli_si128::map.at(imm8)(a);
}
...
namespace extract_epi16 {
	const unordered_map<char, function<short(__m128i)>> map{
...
short sse2_extract_epi16(__m128i a, char imm8) {
	return extract_epi16::map.at(imm8 & 0x7)(a);
}

namespace insert_epi16 {
	const unordered_map<char, function<__m128i(__m128i, short)>> map{
		{0, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 0); }},
		{1, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 1); }},
		{2, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 2); }},
		{3, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 3); }},
		{4, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 4); }},
		{5, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 5); }},
		{6, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 6); }},
		{7, [](__m128i a, short i) { return _mm_insert_epi16(a, i, 7); }},
	};
} // namespace insert_epi16

__m128i sse2_insert_epi16(__m128i a, short i, char imm8) {
	return insert_epi16::map.at(imm8 & 0x7)(a, i);
}
...
void sse2_maskmoveu_si128(__m128i a, __m128i mask, char* mem_addr) { return _mm_maskmoveu_si128(a, mask, (char*)mem_addr); }
...
__m128i sse2_shuffle_epi32(__m128i a, char imm8) {
	return shuffle_epi32::map.at(imm8 & 0x3)(a);
}

namespace shuffle_pd {
	const unordered_map<char, function<__m128d(__m128d, __m128d)>> map{
...
__m128d sse2_shuffle_pd(__m128d a, __m128d b, char imm8) {
	return shuffle_pd::map.at(imm8 & 0x3)(a, b);
}
...
__m128i sse2_shufflehi_epi16(__m128i a, char imm8) {
	return shufflehi_epi16::map.at(imm8 & 0xFF)(a);
}
...
__m128i sse2_shufflelo_epi16(__m128i a, char imm8) {
	return shufflelo_epi16::map.at(imm8 & 0xFF)(a);
}
...
namespace slli_epi16 {
	const unordered_map<char, function<__m128i(__m128i)>> map{
		{ 0, [](__m128i a) { return _mm_slli_epi16(a,  0); }},
		{ 1, [](__m128i a) { return _mm_slli_epi16(a,  1); }},
		{ 2, [](__m128i a) { return _mm_slli_epi16(a,  2); }},
		{ 3, [](__m128i a) { return _mm_slli_epi16(a,  3); }},
		{ 4, [](__m128i a) { return _mm_slli_epi16(a,  4); }},
		{ 5, [](__m128i a) { return _mm_slli_epi16(a,  5); }},
		{ 6, [](__m128i a) { return _mm_slli_epi16(a,  6); }},
		{ 7, [](__m128i a) { return _mm_slli_epi16(a,  7); }},
		{ 8, [](__m128i a) { return _mm_slli_epi16(a,  8); }},
		{ 9, [](__m128i a) { return _mm_slli_epi16(a,  9); }},
		{10, [](__m128i a) { return _mm_slli_epi16(a, 10); }},
		{11, [](__m128i a) { return _mm_slli_epi16(a, 11); }},
		{12, [](__m128i a) { return _mm_slli_epi16(a, 12); }},
		{13, [](__m128i a) { return _mm_slli_epi16(a, 13); }},
		{14, [](__m128i a) { return _mm_slli_epi16(a, 14); }},
		{15, [](__m128i a) { return _mm_slli_epi16(a, 15); }},
		{16, [](__m128i a) { return _mm_slli_epi16(a, 16); }},
	};
} // namespace slli_epi16

__m128i sse2_slli_epi16(__m128i a, char imm8) {
	if (imm8 > 15) imm8 = 16;
	return slli_epi16::map.at(imm8)(a);
}

__m128i sse2_slli_epi32(__m128i a, char imm8) { return _mm_slli_epi32(a, imm8); }

__m128i sse2_slli_epi64(__m128i a, char imm8) { return _mm_slli_epi64(a, imm8); }
...
__m128i sse2_slli_si128(__m128i a, char imm8) {
	if (imm8 > 15) imm8 = 16;
	return slli_si128::map.at(imm8)(a);
}
...
__m128i sse2_srai_epi16(__m128i a, char imm8) { return _mm_srai_epi16(a, imm8); }

__m128i sse2_srai_epi32(__m128i a, char imm8) { return _mm_srai_epi32(a, imm8); }
...
__m128i sse2_srli_epi16(__m128i a, char imm8) { return _mm_srli_epi16(a, imm8); }

__m128i sse2_srli_epi32(__m128i a, char imm8) { return _mm_srli_epi32(a, imm8); }

__m128i sse2_srli_epi64(__m128i a, char imm8) { return _mm_srli_epi64(a, imm8); }
...
__m128i sse2_srli_si128(__m128i a, char imm8) {
	if (imm8 > 15) imm8 = 16;
	return srli_si128::map.at(imm8)(a);
}
...
void sse2_stream_si64(long long* mem_addr, long long a) { return _mm_stream_si64((long long*)mem_addr, a); }
